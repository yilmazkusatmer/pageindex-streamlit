...
async def generate_summaries_for_structure(structure, model=None, batch_size=5, delay=2.0):
    nodes = structure_to_list(structure)
    summaries = []
    for i in range(0, len(nodes), batch_size):
        batch = nodes[i:i + batch_size]
        batch_tasks = [generate_node_summary(node, model=model) for node in batch]
